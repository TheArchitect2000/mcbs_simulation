...
class LightTransaction():
    ...
    def create_transactions(pickUpTime, prevTime):
        ...
        for i in range(Psize):
            ...
            tx.fee= random.expovariate(1/(p.Tfee * (10 ** (2 - tx.level)))) # gas fee update

            #sender.transactionsPool.append(tx)
            # FullTransaction.transaction_prop(tx) ignore transaction propogation time
    ...
    def execute_transactions(miner,currentTime):
        ...
        for i in range(len(LightTransaction.pending_transactions)):

            count=0
            pool= LightTransaction.pending_transactions[i]
            new_class_pool = []
            ...
            while count < len(pool):
                    if  (blocksize >= pool[count].size):
                        ...
                        new_class_pool.append(pool[count])
                    count+=1
